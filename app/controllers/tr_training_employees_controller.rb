class TrTrainingEmployeesController < ApplicationController
  ...
  def index
    ...
    @tr_training_employees = @q.result.includes(:stf_employee, :tr_training_record).joins(tr_training_record: :tr_course).page(params[:page])
  end
  ...
  def tr_training_employee_params
    params.require(:tr_training_employee).permit(:id, :output, :tr_training_record_id, :stf_employee_id, :searchm_cont, stf_employees_attributes: [:id, :clocknum, :name])
  end
end
